...
//-------------------------------------------------------------------------------
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        try {
            digits = values
                    .getInteger(R.styleable.PassCodeView_digits, 4);
            float digitSize = values
                    .getDimension(R.styleable.PassCodeView_digit_size,
                    getResources().getDimension(R.dimen.drawableDimen));
            keyTextSize = values
                    .getDimension(R.styleable.PassCodeView_key_text_size,
                    getResources().getDimension(R.dimen.key_text_size));
            dividerVisible = values
                    .getBoolean(R.styleable.PassCodeView_divider_visible, true);
            digitHorizontalPadding = (int) values
                    .getDimension(R.styleable.PassCodeView_digit_spacing,
                    getResources().getDimension(R.dimen.digit_horizontal_padding));
            digitVerticalPadding =
                    (int) values
                            .getDimension(R.styleable.PassCodeView_digit_vertical_padding,
            ...
            setFilledDrawable(values
                    .getResourceId(R.styleable.PassCodeView_filled_drawable, -1));
            setEmptyDrawable(values
                    .getResourceId(R.styleable.PassCodeView_empty_drawable, -1));
        } catch (Exception e) {
            e.printStackTrace();
        }
        values.recycle();
...
//-------------------------------------------------------------------------------
    //Link google để tra cứu nếu quên
...
//-------------------------------------------------------------------------------
     ...
     */
    private void findKeyPressed(int downEventX, int downEventY
            , int upEventX, int upEventY) {
        for (final KeyRect keyRect : keyRects) {
            if (keyRect
                    .rect
                    .contains(downEventX, downEventY)
                    &&
                    keyRect.
                            rect
                            .contains(upEventX, upEventY)) {
